...
class NodeAdditionalAbsMethods(ProgramNode):
    def __init__():
        super().__init__()
...
def concrete_node() -> ConcreteNode:
    """Returns a ConcreteNode instance for testing."""
    return ConcreteNode()
...
def test_custom_init(concrete_node):
    assert concrete_node._val == 3

def test_base_node_init_core_vals_basic(concrete_node: ConcreteNode):
    node = concrete_node
...
def test_init_invalid_num_children():
    global _token, _num_children, _is_terminal, _is_root, _label
    _token = "<invalid>"
    _is_terminal = False
    _is_root = False
...
def test_properties(concrete_node: ConcreteNode):
    node = concrete_node
